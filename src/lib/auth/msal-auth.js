...
export const getMsalClient = async () => {
  if (!msalClient)
    msalClient = await PublicClientApplication.createPublicClientApplication(
      msalConfig
    );
  ...
  if (accounts.length > 1) {
    console.log(
      "WOAHWOAHWOAH, flere enn en active bruker i MSAL HER! Åpne en ny fane a..."
    );
    msalClient.setActiveAccount(accounts[0]);
  }
  return msalClient;
...
export const login = async (forceLogin = false, loginRequestUrl = "/") => {
  if (import.meta.env.VITE_MOCK_MSAL === "true")
    return {
      account: { username: "demospøkelse@domene.no", name: "Demo Spøkelse" },
      loginRequestUrl,
    };
  ...
  if (loginResponse && !forceLogin) {
    ...
    return {
      ...
      loginRequestUrl: loginResponse.state,
    };
  } else {
